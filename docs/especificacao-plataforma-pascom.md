...
- **Tag "urgente" no Kanban — IMPLEMENTADO:** separada de prioridade (que reflete importância, não janela de tempo). Prioridade "Alta" não implica necessariamente prazo apertado — a tag/badge de urgência cobre especificamente os pedidos de véspera de missa/evento, que precisam se destacar visualmente de forma diferente da prioridade.
  - Coluna `activities.is_urgent` (boolean, default `false`) — sem RLS nem trigger novos, mesmo nível de acesso que já regia `priority`: a policy ampla de `UPDATE` em `activities` (coordenação, responsável, ou qualquer um da área) já cobre
  - Marcação manual, só depois que a atividade já existe no Kanban (`toggleUrgent` em `tarefas/actions.ts`) — não é automática por prazo, não fica disponível no formulário público `/solicitar`, e não aparece na página pública de acompanhamento (`/acompanhar/[token]` não seleciona `priority` nem `is_urgent`)
  - Badge visual novo e deliberadamente fora da escala semântica de cor existente (verde/azul/âmbar/vermelho, já ocupada por prioridade/status/origem): variante `.badge-dark` (sólido escuro + ícone raio) no design system, testada visualmente ao lado do badge "Alta" (vermelho) no Kanban pra confirmar que não seria confundida
  - Bug de UI encontrado e corrigido de carona: `.modal-footer` não tinha `flex-wrap`, e com os 4 botões possíveis (Assumir tarefa, Marcar urgente, seletor de status, Fechar) o botão mais à esquerda ficava cortado pelo `overflow: hidden` do modal em vez de quebrar linha
